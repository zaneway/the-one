...
PACKAGE_NAME="theone-cursor-${VERSION}-${OS_NAME}-${ARCH_NAME}"
...
PY
cp scripts/install-cursor.sh "${PACKAGE_DIR}/install-cursor.sh"
cp scripts/install-cursor.sh "${PACKAGE_DIR}/scripts/install-cursor.sh"
cp -R doc/cursor "${PACKAGE_DIR}/doc/cursor"
...
cp -R drivers/cursor "${PACKAGE_DIR}/drivers/cursor"
cp -R drivers/shared "${PACKAGE_DIR}/drivers/shared"

find "${PACKAGE_DIR}" -name ".DS_Store" -delete
find "${PACKAGE_DIR}/drivers" -name "*_test.py" -delete
chmod +x "${PACKAGE_DIR}/install-cursor.sh" "${PACKAGE_DIR}/scripts/install-cursor.sh"
chmod +x "${PACKAGE_DIR}/bin/theone"
chmod +x "${PACKAGE_DIR}/drivers/cursor/entry.sh" 2>/dev/null || true
chmod +x "${PACKAGE_DIR}/drivers/cursor/hooks/"*.sh 2>/dev/null || true
...
echo "[package-cursor] package: ${PACKAGE_DIR}"
echo "[package-cursor] archive: ${DIST_DIR}/${PACKAGE_NAME}.tar.gz"
