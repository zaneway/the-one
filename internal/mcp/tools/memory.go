...
func toMCPError(err error) *mcp.Error {
	...
	if i := strings.Index(message, ":"); i > 0 {
		...
		switch prefix {
		// 业务错误码，不可重试
		case "VALIDATION_FAILED", "SCOPE_INVALID", "CONTENT_TOO_LARGE", "MEMORY_NOT_FOUND", "STATE_CONFLICT", "FTS_UNAVAILABLE", "SESSION_REQUIRED", "TASK_INVALID", "CAPTURE_UNSUPPORTED", "SESSION_NOT_FOUND", "TASK_NOT_FOUND", "JOB_NOT_FOUND", "CANDIDATE_NOT_FOUND", "MVP_RUN_NOT_FOUND", "MVP_AGENT_CAPABILITY_NOT_FOUND", "MVP_SERVICE_UNAVAILABLE":
			code = prefix
			retryable = false
		// 存储繁忙错误码，可重试
		case "STORAGE_BUSY":
