...
func captureObserveSpec(handler mcp.Handler) mcp.ToolSpec {
	return mcp.ToolSpec{
		...
		InputSchema: mcp.ObjectSchema([]string{"event_type"}, map[string]any{
			...
			"input_summary":        mcp.StringProp("Input body. For turn.completed this is the original user request; for tool events this is tool input text or summary."),
			"output_summary":       mcp.StringProp("Output body. For turn.completed this is the original agent response; for tool events this is tool output text or summary."),
