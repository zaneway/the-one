...
func seedP4B2Memory(t *testing.T, ctx context.Context, store *Store, memoryID string) memory.MemoryItem {
	...
	item := memory.MemoryItem{
		...
		CreatedBy:         "test",
		SourceQuality:     0.8,
		Title:             "retrieval 删除一致性",
		Content:           "删除 memory 时需要清理 retrieval 关联表。",
		NormalizedContent: "删除 memory 时需要清理 retrieval 关联表。",
		SearchText:        "retrieval 删除一致性 删除 memory 时需要清理 retrieval 关联表。",
